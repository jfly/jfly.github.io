...
class Islanders:
    ...
    def is_solved(self):
        ...
        than everyone else."""
        unknown_count = (
            self.heavier_or_normal_count
            + self.lighter_or_normal_count
            + self.clueless_count
        )
    ...
    def __add__(self, other):
        return Islanders(
            heavier_or_normal_count=self.heavier_or_normal_count
            + other.heavier_or_normal_count,
            lighter_or_normal_count=self.lighter_or_normal_count
            + other.lighter_or_normal_count,
...
class ScaleOperation:
    ...
    def __post_init__(self):
        ...
        for weight, count in (
            (self.left_side + self.right_side).to_count_by_weight().items()
        ):
...
def operate_scale(operation: ScaleOperation) -> Dict[ScaleResult, Islanders]:
    ...
    heavier_or_normal_count = (
        left_side.heavier_or_normal_count + left_side.clueless_count
    )
    lighter_or_normal_count = (
        right_side.lighter_or_normal_count + right_side.clueless_count
    )
    if heavier_or_normal_count + lighter_or_normal_count > 0:
        islanders_by_result[ScaleResult.LEFT_SIDE] = Islanders(
            normal_count=islanders.total_count()
            - heavier_or_normal_count
            - lighter_or_normal_count,
            clueless_count=0,
    ...
    heavier_or_normal_count = (
        right_side.heavier_or_normal_count + right_side.clueless_count
    )
    lighter_or_normal_count = (
        left_side.lighter_or_normal_count + left_side.clueless_count
    )
    if heavier_or_normal_count + lighter_or_normal_count > 0:
    ...
    normal_count = (
        not_on_scale.normal_count + left_side.total_count() + right_side.total_count()
    )
        ...
        for clueless_count in range(islanders.clueless_count + 1):
            for left_heavier_count in range(islanders.heavier_or_normal_count + 1):
                for left_lighter_count in range(islanders.lighter_or_normal_count + 1):
                    ...
                    if left_count == 0:
                        continue
                    for right_clueless_count in range(
                        islanders.clueless_count + 1 - clueless_count
                    ):
                        for right_heavier_count in range(
                            islanders.heavier_or_normal_count + 1 - left_heavier_count
                        ):
                            right_lighter_count = left_count - (
                                right_clueless_count + right_heavier_count
                            )
                            if (
                                0
                                <= right_lighter_count
                                <= islanders.lighter_or_normal_count
                                - left_lighter_count
                            ):
                                operations.add(
                                    ScaleOperation(
...
def get_worst_result_score(
    islanders: Islanders, operation: ScaleOperation
) -> Optional[int]:
...
def search(islanders: Islanders) -> ScoredOperation:
    ...
    for operation in get_operations(islanders):
        ...
        if worst_result_score is None:
            continue
        if (
            best_scored_operation is None
            or worst_result_score < best_scored_operation.score
        ):
