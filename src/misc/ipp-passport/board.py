...
class PlacedPiece(namedtuple("PlacedPiece", ["piece", "location"])):
    def board_locations(self):
        ...
        for location_in_piece in piece_pixel_locations:
            ...
            location_in_board = self.location + (
                location_in_piece - northwestmost_piece_location
            )
...
class Board(PixelThing):
    ...
    def can_place_pixel_at(self, location):
        return (
            location not in self.location_to_piece
            and location in self.get_pixel_locations()
        )

    def can_place_piece(self, placed_piece):
        return all(
            self.can_place_pixel_at(location)
            for location in placed_piece.board_locations()
        )
    ...
    def get_unfilled_locations(self):
        return set(
            filter(
                lambda location: self.can_place_pixel_at(location),
                self.get_pixel_locations(),
            )
        )
    ...
    def get_board_with_piece_placed(self, placed_piece):
        ...
        return Board(
            self.to_pixel_str(), name=self.name, placed_pieces=new_placed_pieces
        )
    ...
    def get_all_boards_with_piece_placed(self, piece):
        boards = []
        northwestmost_open_location = northwestmost_location(
            self.get_unfilled_locations()
        )
