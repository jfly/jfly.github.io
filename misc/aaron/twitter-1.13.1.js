    ...
    window.getTwitters = function (target, id, count, options) {
        ...
        ready((function(options, guid) {
            return function () {
                ...
                //<<<var url = 'http://www.twitter.com/statuses/' + (options.withFriends ? 'friends_timeline' : 'user_timeline') + '/' + id + '.json?callback=twitterCallback' + guid + '&count=' + options.count + '&cb=' + Math.random();
				var url = "https://api.twitter.com/1/statuses/user_timeline.json?screen_name=" + id + "&count=" + options.count + "&callback=twitterCallback" + guid;
